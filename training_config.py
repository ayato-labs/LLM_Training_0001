...
# =====================================================================
SEQ_LEN = 512           # コンテキストウィンドウ長。VRAM 4GBで150Mモデルを安全にGPUオンリー稼働させるため512を設定。
MAX_STEPS = -1          # 学習ステップ数 (-1でエポックベースの学習を行います)
NUM_EPOCHS = 3          # 本格学習のためのエポック数 (3エポック、約33時間)
TARGET_TOKENS = 30000000 # 学習の最適化計算で使用するターゲットの想定総トークン数（実際のデータセットと同等）
TARGET_PARAMS = 150000000 # ターゲットとするモデルのパラメータ数 (150M)
