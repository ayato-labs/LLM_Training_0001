# ADR-018: BF16 (BFloat16) 採用による学習安定性向上 / Windowsネイティブ対応
...
**追記 (2026-07-11)**: Windowsネイティブ環境で `0xC0000005` (Access Violation) が発生していたが、DLLロード順序の制御（`torch` 等を先に import）と CUDA版 PyTorch の明示的インストールで解決。Windows環境でも BF16 学習が安定して動作することを確認。
...
- Windowsネイティブ環境で DLL 競合を回避し安定動作
...
- `src/main.py`: `import torch` 等を冒頭で実行し DLL ロード順序を固定
