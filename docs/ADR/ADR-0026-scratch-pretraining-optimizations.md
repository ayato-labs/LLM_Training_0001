...
1. **Scaled Dot Product Attention (SDPA)**:
   Configure models to run with `attn_implementation="sdpa"` (PyTorch 2.0+ standard). This reduces attention memory usage to $O(N)$ and speeds up computation. SDPA is cross-platform (Windows/Linux) and requires no additional packages. (ADR-0026 original used flash_attention_2, migrated to SDPA in ADR-0032 for Windows compatibility)
...
- **Speed**: SDPA and fused optimizers maximize GPU tensor core utilization.
- **Cross-platform**: SDPA works on both Windows and Linux without additional packages.
...
## Related ADRs
- ADR-0032: FlashAttention-2 → SDPA migration
