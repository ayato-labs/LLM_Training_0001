...
def test_scaling_laws_to_training_pipeline():
    ...
    d_tokens = chinchilla_result["computable_tokens"]

    # 2. scaling_config.yaml 相当の生成
    with tempfile.TemporaryDirectory() as tmpdir:
        config_path = Path(tmpdir) / "scaling_config.yaml"
        yaml_content = {
            "metadata": {"computable_tokens": d_tokens},
...
def test_scaling_laws_hpo_batch_size_change_updates_max_steps():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # HPO前: batch=16
        c1 = _normalize_config(dict(raw))
        assert c1["max_steps"] == 1_240_000_000 // (16 * 512)  # 151367

        # HPO後: HPOが batch_size_seqs = 32 を試行
        raw["training"]["batch_size_seqs"] = 32
        c2 = _normalize_config(dict(raw))
        assert c2["max_steps"] == 1_240_000_000 // (32 * 512)  # 75683 (半減)


def test_scaling_laws_computable_tokens_not_overridden_by_hpo():
    """HPOで設定が更新されても computable_tokens が維持される"""
    with tempfile.TemporaryDirectory() as tmpdir:
        config_path = Path(tmpdir) / "scaling_config.yaml"
        yaml_content = {
            "metadata": {"computable_tokens": 1_240_000_000},
